...
- **基礎值**：4（所有店舖的基礎庫存數）
- **Class權重**：A級=1.8，B級=1.4，C級=1.0，D級=1.0
- **Size權重**：XL=2.0，L=1.6，M=1.2，S=0.8，XS=0.8
...
- **權重計算**：4 + (1.8 × 1.6 × 1.2 × 1.0) = 4 + 3.46 ≈ **8**
- **業務意義**：遊客消費頻率高，需要較多庫存應對突發需求

### 範例 2：混合型中型 B 級店舖
- **店舖特徵**：HB12（黃埔），B級，中型（M），混合型（M），香港
- **權重計算**：4 + (1.4 × 1.2 × 1.0 × 1.0) = 4 + 1.68 ≈ **6**
- **業務意義**：客源均勻分佈，需要適度庫存

### 範例 3：本地客源小型 C 級店舖
- **店舖特徵**：HA40（山頂），C級，小型（S），本地型（L），香港
- **權重計算**：4 + (1.0 × 0.8 × 0.9 × 1.0) = 4 + 0.72 ≈ **5**
...
### 目前設定（推薦）
- **特點**：平衡各層权重，Type 差異適中（1.2 → 1.0 → 0.9）
- **適用場景**：大多數常規經營情況

### 保守型
- **特點**：降低所有權重，Type 差異最小（1.05 → 1.0 → 0.98）
- **適用場景**：庫存成本高，銷售較穩定的場景

### 積極型
- **特點**：提高所有權重，Type 差異明顯（1.3 → 1.0 → 0.85）
- **適用場景**：銷售波動大，防缺貨優先的場景
...
### 如果本地客源較穩定
→ 降低 L 的權重，例如 0.9 → 0.85

### 如果想減少庫存總量而保持結構
→ 降低基礎值（例如 4 → 3）同時微調 Type 權重
...
- **v1.0** - 首次加入 Type 權重分類，支援 T/M/L 三層級
- **計算公式**：引入 Type 權重，調整 Class 和 Size 的權重以保持平衡
- **預設值**：T=1.2, M=1.0, L=0.9（遊客區溢價 20%，本地型優惠 10%）
